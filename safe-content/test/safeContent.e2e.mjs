...
test('§5.1: the safe pipeline never imports @mdx-js/mdx compile() or acorn (compiled path unreachable)', async () => {
  ...
  //
  // We match import/require EDGES, not any substring "acorn". In the standalone
  // package (R3-279) the built artifact is `dist/index.js` (+cjs); the same
  // no-evaluator guarantee applies to the bundle the e2e runs and to the dist.
  // Since R3-213's packaging fix, `mdastDeps` is a self-contained esbuild bundle that inlines
