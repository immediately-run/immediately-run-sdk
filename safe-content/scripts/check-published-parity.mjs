 ...
 *
 * `ci.yml`'s safe-content publish step skips when `package.json`'s version
 * already exists, and says so with a `::notice::` — not a failure. That skip is
 * correct (a re-run of an old commit must not try to republish) and it is also
 * how `main` and a published package drift apart without a single red build: a
 * pin or a payload file moves on `main`, the version is never bumped, so the
 * published package keeps the old bytes while the release no-ops silently.
 *
 * The invariant this restores: a skip is benign only when it is a NO-OP. Same
 * version, different declarations or different packed bytes, means the version
 * number is a lie and the fix is to bump it.
 *
 ...
 * same — the port R3-756 owns. It could not be copied blind: safe-content
 * publishes BUILT `dist`, so a payload compare is only sound if the build is
 * reproducible (same input tree, same output bytes), and that is PROVEN first
 * by `scripts/check-build-reproducible.mjs` (two builds of one tree,
 * byte-identical), which runs beside this check in this package's `verify`. The
 * payload compare needs a built `dist/`: without one it is the cannot-answer
 * case (exit 2; `--offline-ok` downgrades it like an unreadable registry — a
 * line says the comparison never ran), never a pass that looks like parity.
 *
 * Unlike the SDK root package, this copy is invoked STRICTLY (no `--offline-ok`)
 * by the release job's already-published branch — ci.yml's safe-content job — so
 * a drift there stops the skip before it no-ops.
 *
 ...
 *   - `immediately-run-cli` — closed by R3-756;
 *   - `immediately-run-sdk/safe-content` (this script) — closed by R3-756.
 *
 ...
 *
 * This package's `verify` (through `check:published`, after `npm run build` —
 * the chain builds first, so the payload half compares real bytes), and the
 * release job's already-published branch runs it strictly (no `--offline-ok`).
 * A version not yet published is the ordinary state everywhere — exit 0, nothing
 * to compare.
 *
 ...
 * (`package.json`, `dist/index.js`, …). Bounded like every other process this
...
function selfTest() {
  ...
  // `npm view @immediately-run/safe-content@0.0.0-nonexistent dependencies --json` — not
  ...
  const e404 = JSON.stringify({
    error: {
      ...
      detail:
        "The requested resource '@immediately-run/safe-content@0.0.0-nonexistent' could not be found or you do not have permission to access it.\n\nNote that you can also install from a\ntarball, folder, http url, or git url.",
    },
  });
  check(
  ...
  // + `FIELDS`), against @immediately-run/safe-content@0.1.1, captured 2026-09-23.
  const real = JSON.stringify({
    dependencies: { '@immediately-run/mdx-plugins': '0.5.0' },
  ...
  try {
    ...
    // package's payload is the BUILT `dist`, so the case runs only where a build
    // exists (`verify` reaches it after `npm run build`; elsewhere it prints an
    // honest SKIP).
    if (existsSync(join(ROOT, 'dist'))) {
      ...
      check(
        ...
        real.get('package.json') === directReal('package.json') &&
          ...
          real.get('dist/index.cjs') === directReal('dist/index.cjs'),
      );
...
try {
  stdout = execFileSync('npm', ['view', spec, ...BLOCKS, ...FIELDS, '--json'], {
    encoding: 'utf8',
    stdio: ['ignore', 'pipe', 'ignore'],
    // The package's only outbound call. Without a bound, a hung registry read
    // holds the job to its ceiling; a kill lands in the catch below and reads as
    // unreadable.
    timeout: 30_000,
  });
} catch (e) {
