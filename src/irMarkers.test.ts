...
describe("validateMarker — the LP-5 host vocabulary allowlist", () => {
  ...
  it("accepts a recognized per-module/per-dep sub-mark, inheriting the aggregate schema", () => {
    expect(validateMarker({ name: "ir.transpile.mod[/src/App.tsx]", at: 2, attrs: { moduleCount: 1 } }))
      .toEqual({ name: "ir.transpile.mod[/src/App.tsx]", at: 2, attrs: { moduleCount: 1 } });
    expect(validateMarker({ name: "ir.deps.pkg[react@18.2.0]", at: 3, attrs: { bytes: 9 } }))
      .toEqual({ name: "ir.deps.pkg[react@18.2.0]", at: 3, attrs: { bytes: 9 } });
  });
