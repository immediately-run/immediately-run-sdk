...
// node_modules/@immediately-run/safe-content/dist/chunk-PZ5AY32C.js
var __defProp2, __export2;
var init_chunk_PZ5AY32C = __esm({
  "node_modules/@immediately-run/safe-content/dist/chunk-PZ5AY32C.js"() {
...
// node_modules/@immediately-run/safe-content/node_modules/@immediately-run/mdx-plugins/dist/index.js
function textSlug(text4) {
  return text4.trim().toLowerCase().replace(/[^\w\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/^-+|-+$/g, "");
}
function sectionId(text4) {
  const m = text4.trim().match(LEADING_TOKEN);
  if (!m) return null;
  const token = m[1];
...
function headingId(text4, options = {}) {
  const sec = options.sectionIds !== false ? sectionId(text4) : null;
  if (sec) return sec;
  const slug = textSlug(text4);
...
function headingText(node2) {
  ...
  const visit2 = (n) => {
    ...
    if (Array.isArray(n.children)) n.children.forEach(visit2);
  };
  if (Array.isArray(node2.children)) node2.children.forEach(visit2);
  return out;
}
function hasAuthorId(node2) {
  const id = node2.data?.hProperties?.id;
...
function resolveCorpusAbsolute(path2, corpusRoot) {
  const inner = normalizeAbsolute(path2);
  ...
  "node_modules/@immediately-run/safe-content/node_modules/@immediately-run/mdx-plugins/dist/index.js"() {
    ...
    remarkHeadingAnchors = (options = {}) => (tree) => {
      ...
      for (const node2 of children) {
        if (node2.type !== "heading") continue;
        if (hasAuthorId(node2)) continue;
        const text4 = headingText(node2);
        const slug = textSlug(text4);
        const sec = sectionEnabled ? sectionId(text4) : null;
        const baseId = headingId(text4, { sectionIds: sectionEnabled });
        const n = seen.get(baseId) ?? 0;
        seen.set(baseId, n + 1);
        const id = n === 0 ? baseId : `${baseId}-${n}`;
        const hProperties = (node2.data ?? (node2.data = {})).hProperties ?? (node2.data.hProperties = {});
        hProperties.id = id;
        if (sec && slug && slug !== id) hProperties["data-slug"] = slug;
        node2.children = [anchorNode(id), ...node2.children ?? []];
      }
    };
    remarkHeadingAnchors_default = remarkHeadingAnchors;
    FS_PREFIX = "$fs:";
    normalizeAbsolute = (path2) => {
      const out = [];
      for (const seg of path2.split("/")) {
        if (seg === "" || seg === ".") continue;
        if (seg === "..") out.pop();
        else out.push(seg);
      }
...
// node_modules/@immediately-run/safe-content/dist/mdastDeps-SZDUOGKZ.js
var mdastDeps_SZDUOGKZ_exports = {};
__export(mdastDeps_SZDUOGKZ_exports, {
  ...
  function build(node2, key, index2, parents) {
    ...
    function visit2() {
      ...
      if (result[0] !== SKIP) {
        let cKey;
        for (cKey in node2) {
          if (own3.call(node2, cKey) && node2[cKey] && typeof node2[cKey] === "object" && // @ts-expect-error: custom esast extension.
...
function factoryTag(effects, ok3, nok, acorn, acornOptions, addResult, allowLazy, tagType, tagMarkerType, tagClosingMarkerType, tagSelfClosingMarker, tagNameType, tagNamePrimaryType, tagNameMemberMarkerType, tagNameMemberType, tagNamePrefixMarkerType, tagNameLocalType, tagExpressionAttributeType, tagExpressionAttributeMarkerType, tagExpressionAttributeValueType, tagAttributeType, tagAttributeNameType, tagAttributeNamePrimaryType, tagAttributeNamePrefixMarkerType, tagAttributeNameLocalType, tagAttributeInitializerMarkerType, tagAttributeValueLiteralType, tagAttributeValueLiteralMarkerType, tagAttributeValueLiteralValueType, tagAttributeValueExpressionType, tagAttributeValueExpressionMarkerType, tagAttributeValueExpressionValueType) {
  ...
  function attributeBefore(code2) {
    ...
    if (code2 === 123) {
      return factoryMdxExpression.call(self, effects, attributeExpressionAfter, tagExpressionAttributeType, tagExpressionAttributeMarkerType, tagExpressionAttributeValueType, acorn, acornOptions, addResult, true, false, allowLazy)(code2);
    }
    if (code2 !== null && code2 >= 0 && start(code2)) {
  ...
  function attributeValueBefore(code2) {
    ...
    if (code2 === 123) {
      return factoryMdxExpression.call(self, effects, attributeValueExpressionAfter, tagAttributeValueExpressionType, tagAttributeValueExpressionMarkerType, tagAttributeValueExpressionValueType, acorn, acornOptions, addResult, false, false, allowLazy)(code2);
    }
  ...
  function crash(code2, at, expect) {
    const error = new VFileMessage("Unexpected " + (code2 === null ? "end of file" : "character `" + (code2 === 96 ? "` ` `" : String.fromCodePoint(code2)) + "` (" + serializeCharCode(code2) + ")") + " " + at + ", expected " + expect, self.now(), "micromark-extension-mdx-jsx:unexpected-" + (code2 === null ? "eof" : "character"));
...
var emptyOptions, characterEntities, own, hasOwnProperty, asciiAlpha, asciiAlphanumeric, asciiAtext, asciiDigit, asciiHexDigit, asciiPunctuation, unicodePunctuation, unicodeWhitespace, content, document, containerConstruct, attention, autolink, blankLine, blockQuote, characterEscape, characterReference, nonLazyContinuation, codeFenced, codeIndented, furtherStart, codeText, SpliceBuffer, content2, continuationConstruct, definition, titleBefore, hardBreakEscape, headingAtx, htmlBlockNames, htmlRawNames, htmlFlow, blankLineBefore, nonLazyContinuationStart, htmlText, labelEnd, resourceConstruct, referenceFullConstruct, referenceCollapsedConstruct, labelStartImage, labelStartLink, lineEnding, thematicBreak, list, listItemPrefixWhitespaceConstruct, indentConstruct, setextUnderline, flow, resolver, string, text, constructs_exports, document2, contentInitial, flowInitial, flow2, string2, text2, insideSpan, attentionMarkers, disable, search, characterEscapeOrReference, own2, startRe, contRe, contReJsx, emptyOptions2, own3, CONTINUE, EXIT, SKIP, VFileMessage, indentSize, trouble, unexpectedEndOfFileHash, unexpectedLazyHash, nonSpreadHash, spreadExtraHash, acornHash, trouble2, characterEntitiesLegacy, characterReferenceInvalid, messages, wwwPrefix, domain, path, trail, emailDomainDotTrail, wwwAutolink, protocolAutolink, emailAutolink, text3, code, indent, EditMap, tasklistCheck, convert, empty2, CONTINUE2, EXIT2, SKIP2;
var init_mdastDeps_SZDUOGKZ = __esm({
  "node_modules/@immediately-run/safe-content/dist/mdastDeps-SZDUOGKZ.js"() {
    init_chunk_PZ5AY32C();
    init_dist();
    emptyOptions = {};
    ...
    emptyOptions2 = {};
    own3 = {}.hasOwnProperty;
    CONTINUE = /* @__PURE__ */ Symbol("continue");
    EXIT = /* @__PURE__ */ Symbol("exit");
    ...
    indentSize = 2;
    trouble = "https://github.com/micromark/micromark-extension-mdx-expression/tree/main/packages/micromark-extension-mdx-expression";
    ...
    trouble2 = "https://github.com/micromark/micromark-extension-mdx-jsx";
...
// node_modules/@immediately-run/safe-content/dist/index.js
var dist_exports = {};
__export(dist_exports, {
  SafeContent: () => SafeContent,
  parseSafeMdast: () => parseSafeMdast,
...
function loadDeps() {
  if (!depsPromise) {
    depsPromise = Promise.resolve().then(() => (init_mdastDeps_SZDUOGKZ(), mdastDeps_SZDUOGKZ_exports)).then((m) => {
      const d = m;
      return {
...
function renderChildren(node2, opts) {
  return (node2.children ?? []).map((c, i) => renderNode(c, opts, i));
}
var HEADING_TAGS = ["h1", "h2", "h3", "h4", "h5", "h6"];
function renderNode(node2, opts, index2 = 0) {
  const key = `n${index2}`;
  switch (node2.type) {
    case "root":
      return createElement(Fragment, { key }, ...renderChildren(node2, opts));
    case "text":
      return renderText(node2.value ?? "", opts);
    case "paragraph":
      return createElement("p", { key }, ...renderChildren(node2, opts));
    case "heading": {
      const tag = HEADING_TAGS[Math.min(Math.max((node2.depth ?? 1) - 1, 0), 5)];
      const hp = node2.data?.hProperties;
      const headingProps = {};
      if (typeof hp?.id === "string") headingProps.id = hp.id;
      if (typeof hp?.["data-slug"] === "string") headingProps["data-slug"] = hp["data-slug"];
      return createElement(tag, { key, ...headingProps }, ...renderChildren(node2, opts));
    }
    case "strong":
      return createElement("strong", { key }, ...renderChildren(node2, opts));
    case "emphasis":
      return createElement("em", { key }, ...renderChildren(node2, opts));
    case "delete":
      return createElement("del", { key }, ...renderChildren(node2, opts));
    case "inlineCode":
      return createElement("code", { key }, node2.value ?? "");
    case "code":
      return createElement("pre", { key }, createElement("code", null, node2.value ?? ""));
    case "blockquote":
      return createElement("blockquote", { key }, ...renderChildren(node2, opts));
    case "list":
      return createElement(node2.ordered ? "ol" : "ul", { key }, ...renderChildren(node2, opts));
    case "listItem":
      return createElement("li", { key }, ...renderChildren(node2, opts));
    case "thematicBreak":
      return createElement("hr", { key });
    case "break":
      return createElement("br", { key });
    case "link": {
      const href = sanitizeUrl(node2.url);
      if (href === void 0) return createElement(Fragment, { key }, ...renderChildren(node2, opts));
      return createElement(
        elementFor("a", opts.components),
        { key, href, title: node2.title ?? void 0 },
        ...renderChildren(node2, opts)
      );
    }
    case "image": {
      const src = sanitizeUrl(node2.url);
      if (src === void 0) return node2.alt ? createElement(Fragment, { key }, node2.alt) : null;
    ...
    // GFM tables.
    case "table":
      return createElement("table", { key }, createElement("tbody", null, ...renderChildren(node2, opts)));
    case "tableRow":
      return createElement("tr", { key }, ...renderChildren(node2, opts));
    case "tableCell":
      return createElement("td", { key }, ...renderChildren(node2, opts));
    ...
    case "html":
      return createElement(Fragment, { key }, node2.value ?? "");
    // JSX `<Component/>` syntax → registry lookup by NAME, literal props only.
    case "mdxJsxFlowElement":
    case "mdxJsxTextElement": {
      const name = typeof node2.name === "string" ? node2.name : "";
      const Component = name ? opts.components?.[name] : void 0;
      const children = renderChildren(node2, opts);
      if (!Component) return createElement(Fragment, { key }, ...children);
      return createElement(Component, { key, ...literalProps(node2.attributes) }, ...children);
    }
    ...
    case "mdxjsEsm":
      return null;
    default:
      return node2.children ? createElement(Fragment, { key }, ...renderChildren(node2, opts)) : null;
  }
}
...
// inline-safe-content.js
var __m = void 0 ?? dist_exports;
var SafeContent2 = __m["SafeContent"];
var parseSafeMdast2 = __m["parseSafeMdast"];
var parseWikiInner2 = __m["parseWikiInner"];
var renderMdast2 = __m["renderMdast"];
var sanitizeUrl2 = __m["sanitizeUrl"];
var splitWikiLinks2 = __m["splitWikiLinks"];
export {
  SafeContent2 as SafeContent,
  parseSafeMdast2 as parseSafeMdast,
  parseWikiInner2 as parseWikiInner,
  renderMdast2 as renderMdast,
  sanitizeUrl2 as sanitizeUrl,
  splitWikiLinks2 as splitWikiLinks
};
