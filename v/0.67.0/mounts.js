...
import { APP_ROOT } from "./_workspace/platform-constants.js";
...
const waitForMount = (query, timeoutMs) => awaitMatchingMount(onMountsChange, query, timeoutMs);
const awaitMatchingMount = (subscribe, query, timeoutMs) => new Promise((resolve, reject) => {
  let unsubscribe;
  let settled = false;
  let timer;
  const stop = () => {
...
const request = async (method, query = {}) => {
  const res = await protocolRequest(SCHEMES[PROTOCOL_SPACES], method, [query]);
  if (!res || res.ok !== true) {
...
const settingsRequest = async (method, query = {}) => {
  const res = await protocolRequest(SCHEMES[PROTOCOL_SETTINGS], method, [query]);
  if (!res || res.ok !== true) {
...
const openSettings = async () => {
  ...
  return waitForMount({ id: mount2.id ?? mount2.path }, SETTINGS_MOUNT_TIMEOUT_MS);
};
const SETTINGS_MOUNT_TIMEOUT_MS = 15e3;
...
const localStoreRequest = async (method, query = {}) => {
  const res = await protocolRequest(SCHEMES[PROTOCOL_LOCALSTORE], method, [query]);
  if (!res || res.ok !== true) {
    const err = new Error(res?.message ?? "localstore request failed");
    err.code = res?.code ?? "unknown";
    throw err;
  }
  return res.data;
};
const openLocalStore = async () => {
  const mount2 = await localStoreRequest("open");
  return waitForMount({ id: mount2.id ?? mount2.path }, LOCALSTORE_MOUNT_TIMEOUT_MS);
};
const LOCALSTORE_MOUNT_TIMEOUT_MS = 15e3;
